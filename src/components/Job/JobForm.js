...
export default (props) => {
    ...
    const redirectToCheckout2 = async (req, res) => {
        if(req.method === "POST"){
            try{
                const params = {
                    submit_type: 'pay',
    ...
    const [product] = React.useState({
        name: "Job Posting",
        price: 30 * 100,
        description: "Job Posting for 30 Days"
      });

        async function handleToken(token, addresses) {
            const response = await axios.post(
                "https://btycwr.sse.codesandbox.io/checkout",
              { token, product }
            );
          ...
          async function stripePayment2() {
            ...
            await axios.post(
                 "https://main.dqqa3112ms9x4.amplifyapp.com/form/create-checkout-session",
                 { price: "test" }
             ).then((res)=>{
    ...
    return(
      ...
      {/* <button type="submit">Checkout</button>*/}
                   ...
                   {/*  <button onClick={redirectToCheckout2}>Pay Bitch</button>  */}
                   
                   {/* <StripeCheckout
                stripeKey="pk_test_51Iq7GdLxbwyf0mci23QkU9lOoNh54TXrBGKt6UJx9dvhIg3bXOZP42ogc7hDS9lqhWfWlA067BkWcBBx9abufgqD00e2l4OGB1"
                token={handleToken}
                billingAddress
                shippingAddress
